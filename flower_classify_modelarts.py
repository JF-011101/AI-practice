...
from tensorflow.python.keras import layers, optimizers, Sequential  #从tensorflow.keras模块下导入layers，optimizers, datasets, Sequential等方法
...
(x_train, x_val, y_train, y_val) = train_test_split(data, label, test_size=0.20, random_state=seed) # Todo 拆分数据集一部分为训练集一部分为验证集，拆分比例可调整
...
# Todo 自行实现模型结构
model = Sequential([
  layers.Conv2D(32, 5, padding='same', activation='relu', input_shape=(w, h, c)),
...
opt = optimizers.adam_v2.Adam(learning_rate=0.0015)   #使用Adam优化器，优化模型参数。lr(learning rate, 学习率)
...
model.fit(x_train, y_train, epochs=30, validation_data=(x_val, y_val), batch_size=30, verbose=2)
#输出模型的结构和参数量
model.summary()
